...
private function update_post_content($content, $today_heading, $links, $config) {
    ...
    // If no links are found, return the content unchanged
    if (empty($links)) {
        return ['content' => $content, 'modified' => $content_modified];
    }
    
    // Find the position of the first <h2> tag
    if (preg_match('/<h2[^>]*>/', $content, $matches, PREG_OFFSET_CAPTURE)) {
        $first_h2_pos = $matches[0][1];
        
        // Find the end of the first <h2> tag
        $first_h2_end = strpos($content, '>', $first_h2_pos) + 1;
        
        // Prepare the new section ONLY if links exist
        // Note: We've removed the $today_heading from being automatically added
        $new_section = "\n" . $this->generate_links_html($links, $config['link_text']);
        
        // Insert the new section right after the first <h2> tag
        $content = substr_replace($content, $new_section, $first_h2_end, 0);
        
        custom_log("Added " . count($links) . " new links after first <h2> tag");
...
private function generate_links_html($links, $link_text) {
    ...
    // Generate today's heading along with the links
    $today_date = date($this->date_formats['display']);
    $html = $this->generate_heading($today_date);
...
public function run_updates() {
    ...
    foreach ($this->post_configs as $post_id => $config) {
        ...
        if (!empty($links)) {
            // Generate today's heading ONLY if links exist
